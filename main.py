...
def get_debt_amount(number, api_token):
    ...
    try:
        ...
        print(f'API request failed for number {number}')
        return None
...
#Load the Excel file
try:
    df = pd.read_excel('E:\PythonProjects\\fssp_api\\numbers.xlsx')
except FileNotFoundError:
    print('Error: numbers.xlsx file not found')
    exit()
...
try:
    df.to_excel('E:\PythonProjects\\fssp_api\\numbers_with_debt.xlsx', index=False)
